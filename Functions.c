...
void create_Candidat_file(FILE*f,char*name)
{
	...
	fprintf(f, "\n%s\t%s\t%d\t%f\t%c\t%f\n",fiche1.nom, fiche1.prenom, fiche1.age, fiche1.poids, fiche1.sexe, fiche1.taille);
...
void create_Groupe_file(FILE*f,char*name)
{
	...
	for(i=0;i<5;i++){
		...
		fprintf(f, "\n%s\t%s\t%s\n",fichegrp1[i].nom, fichegrp1[i].prenom, fichegrp1[i].ville);
...
void add_Candidat(FILE*f,char*name)
{
	...
	fprintf(f, "\n%s\t%s\t%d\t%f\t%c\t%f\n",fiche2.nom, fiche2.prenom, fiche2.age, fiche2.poids, fiche2.sexe, fiche2.taille);
...
void add_Groupe(FILE*f,char*name){
	...
	for(i=0;i<5;i++){
		...
		fprintf(f, "\n%s\t%s\t%s\n",fichegrp2[i].nom, fichegrp2[i].prenom, fichegrp2[i].ville);
...
int seek_Candidat(FILE*f,char*name,char*seek1name,char*seek2name)
{
	...
	printf("\nERREUR, CE FICHIER N'EXISTE PAS\n");
		...
		// tant que le curseur n'est pas a la fin de fichier  faire	
		while(fscanf(f, "\n%s\t%s\t%d\t%f\t%c\t%f\n",fiche3.nom, 
		fiche3.prenom, &fiche3.age, &fiche3.poids, &fiche3.sexe, &fiche3.taille) != EOF){
...
void deleting_Candidat(FILE*f,char*name)
{
	...
	while(fscanf(f, "\n%s\t%s\t%d\t%f\t%c\t%f\n",fiche4.nom, 
		fiche4.prenom, &fiche4.age, &fiche4.poids, &fiche4.sexe, &fiche4.taille) != EOF)
	{
		...
		if((strcmp(nom1,fiche4.nom)!=0)&&(strcmp(prenom1,fiche4.prenom)!=0))
		{
			...
			fprintf(fichiertemp, "\n%s\t%s\t%d\t%f\t%c\t%f\n",fiche4.nom, fiche4.prenom, fiche4.age, fiche4.poids,
			 fiche4.sexe, fiche4.taille);
...
void view_file_Groupe2(FILE*f,char*name){
	...
	if (f==NULL) printf("\nERREUR, CE FICHIER N'EXISTE PAS\n");
		...
		while(fscanf(f, "\n%s\t%s\t%s\n", fichegrp.nom, fichegrp.prenom, fichegrp.ville) != EOF){
			printf("\nCandidat numero %d:\n",compteur+1);
...
void modify_Candidat(FILE*f,char*name){
	...
	fichiertemp=fopen("temmp.txt","w");
	while(fscanf(f, "\n%s\t%s\t%d\t%f\t%c\t%f\n",fiche5.nom, 
		fiche5.prenom, &fiche5.age, &fiche5.poids, &fiche5.sexe, &fiche5.taille) != EOF)
	{
		...
		if((strcmp(nom1,fiche5.nom)!=0)||(strcmp(prenom1,fiche5.prenom)!=0))
			...
			fprintf(fichiertemp, "\n%s\t%s\t%d\t%f\t%c\t%f\n",fiche5.nom, fiche5.prenom, fiche5.age, fiche5.poids,
			 fiche5.sexe, fiche5.taille);
			}
		}
	do{
	...
	fprintf(fichiertemp, "\n%s\t%s\t%d\t%f\t%c\t%f\n",nvlfiche1.nom, nvlfiche1.prenom, nvlfiche1.age, nvlfiche1.poids,
			 nvlfiche1.sexe, nvlfiche1.taille);
	//fermeture de fichier f
	fclose(f);
	//fermeture de fichier temp
	fclose(fichiertemp);
...
void alphab_display(FILE*f,char*name){
	...
	while(fscanf(f, "\n%s\t%s\t%d\t%f\t%c\t%f\n",fiche6.nom, 
		fiche6.prenom, &fiche6.age, &fiche6.poids, &fiche6.sexe, &fiche6.taille) != EOF){
		nvlfiche2[i]=fiche6;
	...
	for(i=0;i<=nbrcandidat;i++){
		...
		printf("\nCandidat %d: Nom: %s\nPrenom:%s\nAge:%d\nTaille:%.2f\nPoids:%.2f\nSexe:%c",
		m+1,nvlfiche2[i].nom,nvlfiche2[i].prenom,nvlfiche2[i].age,t[i],p[i],nvlfiche2[i].sexe);
		printf("\n===============================\n");
		m++;}
	}
	fclose(f);
